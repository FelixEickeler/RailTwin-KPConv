...
class RTIB3p(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        self.work_dir = Path('/home/point_warrior/dataset/pysical/train/16classes_1')

        # Type of task conducted on this dataset
        self.dataset_task = 'cloud_segmentation'

        # Update number of class and data task in configuration
        self.config.num_classes = self.num_classes - len(self.ignored_labels)
        self.config.dataset_task = self.dataset_task
        ...
        for ply in self.path.glob("**/*.ply"):
            if ply.stem.find("_quintile_1") != -1 or ply.stem.find("_quintile_2") != -1:
                continue
