...
class InferenceDataset(PointCloudDataset):
    """Class to handle S3DIS dataset."""

    def __init__(self, config, cloud_path, tree_path, use_potentials=True):
        PointCloudDataset.__init__(self, 'Inference')
        self.config = config
        self.potentials = None

        # Dict from labels to names
        label_from_json = self.config.labels
        to_ignore = self.config.ignored_labels
        ...
        self.init_labels()
        self.config_path = self.config.saving_path
        self.tukey_update_crop = self.config.tukey_update_crop
        self.path = cloud_path.parent
        self.dataset_task = config.dataset_task
        self.set = "inference"
        self.use_potentials = use_potentials


        # Proportion of validation scenes, set by whom and when ?
        self.cloud_names = [cloud_path]
        self.tree_names = [tree_path]
        self.all_splits = [0]
        ...
        # Initiate containers
        self.input_trees = []
        for tn in self.tree_names:
            with open(tn, 'rb') as f:
                self.input_trees.append(pickle.load(f))
        self.input_features = []
        self.input_labels = []

        # filled by initialize_potentials
        self.pot_trees = []
        self.num_clouds = []
        self.test_proj = []
        ...
        # expected_N
        # TODO expect some things
        self.config.expected_N = 1500000
        return
    ...
    def initialize_potentials(self):
        if self.use_potentials:
            ...
            pot_dl = self.config.in_radius / self.config.pot_sample_mult
            cloud_ind = 0

            for i, file_path in enumerate(self.tree_names):
                coarse_KDTree_file = file_path.parent / '{:s}_coarse.pkl'.format(file_path.stem)
                ...
                cloud_ind += 1
                self.potentials += [torch.from_numpy(np.random.rand(search_tree.data.shape[0]) * 1e-3)]
        ...
        while True:
            ...
            with self.worker_lock:
                ...
                if self.set != 'ERF':
                    ...
                    tukeys[d2s*self.tukey_update_crop > np.square(self.config.in_radius)] = 0
            ...
            if n < 2:
                failed_attempts += 1
                if failed_attempts > 10 * self.config.batch_num:
            ...
            if self.set in ['test', 'ERF']:
                ...
                input_labels = self.input_labels[cloud_ind][input_inds][:, self.config.label_selector]
            ...
            input_features = np.hstack((input_features[:, :1], input_features[:, 1:2])).astype(np.float32)
    ...
    def random_item(self, batch_i):
        ...
        while True:
            ...
            # Collect labels and colors
            input_points = (points[input_inds] - center_point).astype(np.float32)
            input_colors = self.input_features[cloud_ind][input_inds]
            if self.set in ['test', 'ERF']:
                input_labels = np.zeros(input_points.shape[0])
            else:
...
class InferenceSampler(Sampler):
    ...
    def __init__(self, dataset: InferenceDataset):
        ...
        print(f"InferenceSampler selected: \n"
              f"\t\t\t\t expected_N: {dataset.config.expected_N} \n"
              f"\t\t\t\t batch_size: {dataset.config.batch_num}")
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        if redo:
            ...
            try:
                expected_N = self.dataset.config.expected_N
            except:
                expected_N = 20000
            ...
            for epoch in range((sample_batches // self.N) + 1):
                for batch_i, batch in enumerate(dataloader):
                    ...
                    # finer low pass filter when closing in
                    fine_tune = np.abs(estim_b - target_b)
                    if len(finer) < 1 and fine_tune < 1:
                        low_pass_T = 100
                        finer.append(True)
                    elif len(finer) < 2 and fine_tune < 0.2:
                        low_pass_T = 1000
                        finer.append(True)

                    # Convergence
                    break_condition = np.max(np.abs(smooth_errors))
                    if finer and break_condition < converge_threshold:
                        breaking = True
                        break
                    ...
                    if verbose and (t - last_display) > 1.0:
                        last_display = t
                        message = 'Step {:5d}  estim_b ={:5.2f} batch_limit ={:7d} break_condition = {:5.3f}'
                        print(message.format(i,
                                             estim_b,
                                             int(self.dataset.batch_limit),
                                             break_condition))
                    ...
                    # self.dataset.config.expected_N = estim_b
