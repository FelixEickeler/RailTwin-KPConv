...
class RTIB3p(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        # Dataset folder
        self.path = Path('/home/point_warrior/dataset/<dataset_path>')
        self.work_dir = Path('/home/point_warrior/workdir')
        ...
        for i, ply in enumerate(self.path.glob("*.ply")):
            self.cloud_names.append(ply.stem)
            self.all_splits.append(i)
            if ply.stem.find("_4_unified") != -1:
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        if self.config.in_features_dim == 1:
            ...
            raise ValueError(f'Only accepted input dimensions are 1, 4 and 7 (without and with XYZ) not {self.config.in_features_dim}')
...
class RTIB3pSampler(Sampler):
    """Sampler for S3DIS"""

    def __init__(self, dataset: RTIB3p):
        ...
        print(f"RTIB3pSampler selected: \n"
