...
class RTIB3p(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        # Dataset folder
        self.path = Path('/home/point_warrior/dataset/synth/rtib3+_16classes')
        self.work_dir = Path('/home/point_warrior/dataset/synth/train/16classes')
        ...
        for i, ply in enumerate(self.path.glob("**/*.ply")):
            ...
            if ply.stem.find("_quintile_4") != -1:
        ...
        self.files = []
        for i, f in enumerate(self.path.glob("**/*.ply")):
